...
#查数据串口数据标题是否符合定义的标题，数据的标题可以修改DEF_TITLE_STRING
...
def Motion_Assign():
    ...
    :param motion: 动作名称应丄7'motion_name'列表中的个元
    :param folder_path: 包含文件的目录路径
    :return: 匹配文件中找到的朢大数字，如果没有匹配的文件则返
    """
...
def main():
    ...
    while user_input == 'y':
        ...
        while recorded_count < data_set_max:
            received = serial_using.readall().decode("ascii",errors="ignore")
            if len(received) != 0:
